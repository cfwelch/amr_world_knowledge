...
class Parser(object):
    ...
    rty = None #
    train_grad_prod = 0.0;
    train_wrong_action = 0;
    train_correct_action = 0;
    train_wrong_label = 0;
    train_correct_label = 0;
    test_grad_prod = 0.0;
    test_wrong_action = 0;
    test_correct_action = 0;
    test_wrong_label = 0;
    test_correct_label = 0;
    ...
    ## This is where I should add the loss function for acts and labels back in (instead of the graphstate get_score function).
    def get_best_act_loss(self,scores,actions,g_a_index,g_l_index):
        best_label_index = None;
        scores_action_loss = np.copy(scores);
        #print("Scores input array: " + str(scores));
        print("Action array before loss: " + str(map(np.amax,scores_action_loss)));
        for _i in range(0, len(scores_action_loss)):
            if _i != g_a_index:
                print("adding loss to action " + str(_i) + "...");
                scores_action_loss[_i] += 1.0;# action loss
        print("Action array after loss: " + str(map(np.amax,scores_action_loss)));
        best_act_ind = np.argmax(map(np.amax,scores_action_loss));
        print("Gold action index: " + str(g_a_index));
        print("Best action index: " + str(best_act_ind));
        best_act = actions[best_act_ind];
        set_grad_prod = False;
        objective = 0;
        if best_act_ind != g_a_index:
            Parser.train_wrong_action += 1;
        else:
            Parser.train_correct_action += 1;
        if best_act['type'] in ACTION_WITH_EDGE or best_act['type'] in ACTION_WITH_TAG:
            scores_label_loss = np.copy(scores[best_act_ind]);
            print("Scores label: " + str(scores_label_loss));
            print("Gold label index: " + str(g_l_index));
            # It doesn't make sense to add loss to labels that don't have the correct action to begin with...
            if best_act_ind == g_a_index:
                for _i in range(0, len(scores_label_loss)):
                    if _i != g_l_index:
                        scores_label_loss[_i] += 1.0;# label loss
            print("Scores label loss: " + str(scores_label_loss));
            best_label_index = scores_label_loss.argmax();
            if g_a_index < len(scores):
                objective = scores_label_loss[best_label_index] - scores[g_a_index][g_l_index];
            set_grad_prod = True;
            if g_l_index != best_label_index:
                Parser.train_wrong_label += 1;
            else:
                Parser.train_correct_label += 1;
        if not set_grad_prod:
            if g_a_index < len(scores):
                objective = np.max(scores_action_loss[best_act_ind]) - np.max(scores[g_a_index]);
        Parser.train_grad_prod += objective;
        print("Objective score is: " + str(objective));
        return best_act_ind, best_label_index;

    def get_best_act(self,scores,actions,g_a_index,g_l_index):
        best_label_index = None;
        best_act_ind = np.argmax(map(np.amax,scores));
        best_act = actions[best_act_ind];
        if best_act_ind != g_a_index:
            Parser.test_wrong_action += 1;
        else:
            Parser.test_correct_action += 1;
        if best_act['type'] in ACTION_WITH_EDGE or best_act['type'] in ACTION_WITH_TAG:
            best_label_index = scores[best_act_ind].argmax();
            if g_l_index != best_label_index:
                Parser.test_wrong_label += 1;
            else:
                Parser.test_correct_label += 1;
        return best_act_ind, best_label_index;
    ...
    def get_label_index(act,label):
        if act['type'] in ACTION_WITH_EDGE:
            index = Parser.State.model.rel_codebook.get_index(label)[0] if label is not None else 0
        elif act['type'] in ACTION_WITH_TAG:
            index = Parser.State.model.tag_codebook['ABTTag'].get_index(label)[0]
        else:
            index = 0
    ...
    def parse_corpus_train(self, instances, interval=500):
        ...
        print("Initial model weight: " + str(Parser.State.model.weight));
        ...
        print("(TRAIN) The sum of gradients is: " + str(Parser.train_grad_prod));
        print("(TRAIN) Number of wrong actions: " + str(Parser.train_wrong_action));
        print("(TRAIN) Number of wrong labels: " + str(Parser.train_wrong_label));
        print("(TRAIN) Number of correct actions: " + str(Parser.train_correct_action));
        print("(TRAIN) Number of correct labels: " + str(Parser.train_correct_label));
        Parser.train_grad_prod = 0.0;
        Parser.train_wrong_label = 0;
        Parser.train_wrong_action = 0;
        Parser.train_correct_label = 0;
        Parser.train_correct_action = 0;
    ...
    def parse_corpus_test(self, instances, EVAL=False):
        ...
        if EVAL:
            ...
            # printing wrong action and label counts
            print("(TEST) The sum of gradients is: " + str(Parser.test_grad_prod));
            print("(TEST) Number of wrong actions: " + str(Parser.test_wrong_action));
            print("(TEST) Number of wrong labels: " + str(Parser.test_wrong_label));
            print("(TEST) Number of correct actions: " + str(Parser.test_correct_action));
            print("(TEST) Number of correct labels: " + str(Parser.test_correct_label));
            Parser.test_grad_prod = 0.0;
            Parser.test_wrong_label = 0;
            Parser.test_wrong_action = 0;
            Parser.test_correct_label = 0;
            Parser.test_correct_action = 0;
        ...
        while not state.is_terminal():
            ...
            if len(actions) == 1:
                ...
                if train:
                    ...
                    GraphState.num_updates = self.perceptron.num_updates;
                    scores = map(state.get_score,(act for act in actions),features);
                    ...
                    print("Gold act: " + str(gold_act));
                    print("Gold label: " + str(gold_label));

                    try:
                        gold_act_ind = actions.index(gold_act)
                        print("Gold act index: " + str(gold_act_ind));
                    except ValueError:
                        print('WARNING: gold action %s not in possible action set %s'%(str(gold_act),str(actions)));
                        ...
                        new_feats = state.make_feat(gold_act);
                    ...
                    best_act_ind, best_label_index = self.get_best_act_loss(scores,actions,gold_act_ind,gold_label_index);
                    best_act = actions[best_act_ind]
                    ...
                    Parser.State.model.num_updates = GraphState.num_updates;
                    
                    #print "Done update, %s"%(round(time.time()-start_time,2))
                    #raw_input('ENTER TO CONTINUE')
                else:
                    features = map(state.make_feat,actions)
                    scores = map(state.get_score,(act for act in actions),features,[train]*len(actions));
                    ...
                    if ref_graph is not None:
                        ...
                        if gold_act in actions:
                            gold_act_ind = actions.index(gold_act)
                            gold_label_index = Parser.State.model.rel_codebook.get_index(gold_label)[0]
                            #self.output_weight(gold_act_ind,gold_label_index,features,actions)

                    best_act_ind, best_label_index = self.get_best_act(scores,actions,gold_act_ind,gold_label_index);
                    best_act = actions[best_act_ind];
                    best_label = Parser.get_index_label(best_act,best_label_index);
    ...
    def output_weight(self,act_ind,label_index,feats,actions):
        ...
        act_idx = GraphState.model.class_codebook.get_index(actions[act_ind]['type'])[0]
