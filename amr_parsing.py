...
def main():
    ...
    # test deterministic oracle 
        ...
        #this is kind of cool to look at
        #print('token_to_concept table: ' + str(dict([(k,list(v)) for k,v in model.token_to_concept_table.items()])));
        ...
        for iter in xrange(1,args.iterations+1):
            ...
            #parser.perceptron.average_weight()# shouldn't need to do this now that it's not really a perceptron
        ...
        model.save_model(args.model+'.m');# no need to save a 'best' model... just save the one after all iterations because it should be building on past iterations
        ...
        GraphState.num_updates = model.num_updates;
        print("Loaded graphstate num_updates: " + str(GraphState.num_updates));
        parser = Parser(model=model,oracle_type=DET_T2G_ORACLE_ABT,action_type=args.actionset,verbose=args.verbose,elog=experiment_log)
